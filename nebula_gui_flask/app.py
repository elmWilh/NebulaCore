...
def _core_request_with_session(method: str, endpoint: str, user_id: str, db_name: str, params=None, json_data=None):
    url = f"{bridge.core_url}{endpoint}"
    cookies = {"nebula_session": f"{user_id}:{db_name}"}
    r = requests.request(
        method=method,
        url=url,
        params=params,
...
def _run_deploy_job(job_id: str, payload: dict, started_by: str, started_db: str):
    try:
...
def api_deploy_container_start():
    ...
    started_db = session.get('db_name', 'system.db')
    now = time.time()
    ...
    worker = threading.Thread(
        ...
        args=(job_id, payload, started_by, started_db),
        daemon=True
    )
    worker.start()
...
def api_user_detail(username):
    ...
    res, code = bridge.proxy_request("GET", f"/users/detail/{username}")
    return jsonify(res), code
